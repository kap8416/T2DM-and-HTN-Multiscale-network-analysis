...
gse <- getGEO("GSE25724", GSEMatrix =TRUE,getGPL=FALSE) #or# gse <- setwd("C:/Users/lcz/Documents/GSE24572")
datMeta <- pData(gse[[1]])
...
datMeta$Dx[rownames(datMeta) %in% c("GSM631755", "GSM631756", "GSM631757", "GSM631758", "GSM631759", "GSM631760", "GSM631761")] <- "CTL"
datMeta$Dx[rownames(datMeta) %in% c("GSM631762", "GSM631763", "GSM631764", "GSM631765", "GSM631766", "GSM631767" )] <- "T2D"
...
CR <- collapseRows(datExpr, rowGroup = geneDat$ensembl_gene_id, rowID = geneDat$affy_hg_u133a_2, method = "MaxMean")
CR$datETcollapsed
idx <- match(CR$group2row[,"selectedRowID"], geneDat$"affy_hg_u133a_2")
geneDat <- geneDat[idx,]
...
t2d_samples <- c("GSM631762.CEL.gz", "GSM631763.CEL.gz", "GSM631764.CEL.gz", "GSM631765.CEL.gz", "GSM631766.CEL.gz", "GSM631767.CEL.gz" )  # Diabéticos
ctl_samples <- c("GSM631755.CEL.gz", "GSM631756.CEL.gz", "GSM631757.CEL.gz", "GSM631758.CEL.gz", "GSM631759.CEL.gz", "GSM631760.CEL.gz", "GSM631761.CEL.gz")  # Controles

# Calcular la media para cada TF en los grupos HTA y CTL
tf_means <- data.frame(
  TF = rownames(tf_activity),
  T2D_mean = rowMeans(tf_activity[, t2d_samples]),
  CTL_mean = rowMeans(tf_activity[, ctl_samples]),
  stringsAsFactors = FALSE
)

# Aplicar la prueba t para comparar las medias de los grupos
p_values <- apply(tf_activity, 1, function(x) t.test(x[t2d_samples], x[ctl_samples])$p.value)
